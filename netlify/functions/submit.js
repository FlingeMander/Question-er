...
exports.handler = async function(event) {
  if (event.httpMethod === 'OPTIONS') {
    return { statusCode: 200, headers: { 'Access-Control-Allow-Methods': 'POST, OPTIONS', 'Access-Control-Allow-Headers': 'Content-Type' }, body: '' };
  }
  ...
  if (!gmailUser || !gmailPassword || !receiverEmail) {
    return json(500, { error: 'Configuration email manquante. Vérifiez GMAIL_USER, GMAIL_APP_PASSWORD et RECEIVER_EMAIL.' });
  }
  ...
  try {
    ...
    return json(500, { error: 'Impossible d’envoyer l’email. Vérifiez le Gmail App Password et les variables d’environnement.' });
  }
};
